<?php
// ucp/map.php - ÉLŐ SZERVER TÉRKÉP (SZÁMLÁLÓKKAL)
...
<head>
    ...
    <style>
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .map-legend { display: flex; gap: 15px; background: rgba(0,0,0,0.2); padding: 10px 20px; border-radius: 8px; border: 1px solid rgba(255,255,255,0.05); font-size: 12px; font-weight: bold; margin-bottom: 15px; justify-content: center; flex-wrap: wrap; }
        .legend-item { display: flex; align-items: center; gap: 8px; color: #d1d5db; }
        ...
        /* ÚJ BŐVÍTETT SZÍNEK ÉS IKONOK */
        .blip.player { background-color: #3b82f6; border: 2px solid #fff; z-index: 12;} 
        .blip.npc { background-color: #ef4444; border: 2px solid #fff; width: 10px; height: 10px;} 
        .blip.vehicle { background-color: #f59e0b; border: 2px solid #fff; z-index: 11;} 
        .blip.trash { background-color: #10b981; border: 1px solid #fff; width: 8px; height: 8px; border-radius: 2px; z-index: 5;} 
        .blip.casino { background-color: #8b5cf6; border: 2px solid #fff; z-index: 6;} 
        .blip.interior { background-color: #ec4899; border: 2px solid #fff; z-index: 7;} /* Rózsaszín */
        .blip.safe { background-color: #06b6d4; border: 2px solid #fff; width: 10px; height: 10px; z-index: 8;} /* Cián */
        .blip.generic { background-color: #64748b; border: 1px solid #fff; width: 8px; height: 8px; z-index: 4;} 
...
<body>
    ...
    <div class="container">
        <div class="welcome-box">
            ...
            <div style="color: #9ca3af; font-size: 14px;">Látható jelölők összesen: <span id="marker-count" style="color:#fff; font-weight:bold;">0 db</span></div>
        </div>
        ...
        <div class="map-legend">
            ...
            <div class="legend-item"><div class="legend-dot" style="background: #ef4444; box-shadow: 0 0 5px #ef4444;"></div> NPC: <span id="count-npc" style="color:#fff;">0</span></div>
            ...
            <div class="legend-item"><div class="legend-dot" style="background: #8b5cf6;"></div> Casino: <span id="count-blackjack" style="color:#fff;">0</span></div>
            <div class="legend-item"><div class="legend-dot" style="background: #ec4899;"></div> Interior: <span id="count-interior" style="color:#fff;">0</span></div>
            ...
            <div class="legend-item"><div class="legend-dot" style="background: #64748b;"></div> Egyéb: <span id="count-generic" style="color:#fff;">0</span></div>
        </div>
    ...
    <script>
        ...
        async function fetchMapData() {
            try {
                ...
                let totalCount = 0;
                let counts = { player: 0, vehicle: 0, npc: 0, trash: 0, blackjack: 0, interior: 0, safe: 0, generic: 0 };
                ...
                data.forEach(item => {
                    ...
                    if(leftPercent >= 0 && leftPercent <= 100 && topPercent >= 0 && topPercent <= 100) {
                        ...
                        // KATEGORIZÁLÁS ÉS SZÁMOLÁS
                        if (isMe && type === 'characters') {
                            ...
                            dot.className = 'blip interior';
                            counts.interior++;
                        } else if (type === 'safes' || type === 'atms') {
                        ...
                        dot.innerHTML = `<div class="blip-tooltip">${nameFormatted} <br><span style="color:#9ca3af; font-weight:normal; font-size:9px;">[Típus: ${type} | ID: #${charId}]</span></div>`;
